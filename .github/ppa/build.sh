...
for act in "${@}"; do
    ...
    create)
        ...
        sudo apt-get -y install pbuilder pbuilder-scripts debootstrap devscripts dh-make dput dh-python
    ...
    sync)
        rsync -aq \
            ...
            ../../third_party \
            "$rep/"
        sed -i "s/export CLINGODL_BUILD_REVISION =.*/export CLINGODL_BUILD_REVISION = $(git rev-parse --short HEAD)/" "${rep}/debian/rules"
        ;;
    changes)
        VERSION="$(sed -n '/#define CLINGODL_VERSION "/s/.*"\([0-9]\+\.[0-9\+]\.[0-9]\+\)".*/\1/p' ../../lib/c-api/include/clingo-dl.h)"
...
clingo-dl (${VERSION}${SUFFIX}) ${rep}; urgency=medium
...
EOF
    ...
    build)
        if [[ "${BUILD_TYPE}" == "ppa" ]]; then
            ...
            (
                ...
                sed -i '/\.buildinfo$/d' "../clingo-dl_${VERSION}_source.changes"
                debsign --re-sign -k744d959e10f5ad73f9cf17cc1d150536980033d5 "../clingo-dl_${VERSION}_source.changes"
            )
        else
            (
                cd "${rep}"
    ...
    put)
        VERSION="$(head -n 1 "${rep}/debian/changelog" | grep -o '[0-9]\+\.[0-9]\+\.[0-9]\+\(-[a-z0-9]\+\)\?')"
        dput "ppa:potassco/${ref}" "clingo-dl_${VERSION}_source.changes"
        ;;
    clean)
        rm -rf \
            "${rep}"/app \
