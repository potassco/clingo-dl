...
def adjust_version(build_number, git_hash):
    ...
    version = None
    with open("lib/c-api/include/clingo-dl.h", encoding="utf-8") as fh:
        for line in fh:
            m = re.match(r'#define CLINGODL_VERSION "([0-9]+\.[0-9]+\.[0-9]+)"', line)
            if m is not None:
                version = m.group(1)
    ...
    pyproject["project"]["urls"]["Source"] = (
        f"https://github.com/potassco/clingo-dl/tree/{git_hash}"
    )
