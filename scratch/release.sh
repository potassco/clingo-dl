...
function package() {
    ...
    run_rsync "../clingo/scratch/lua/CMakeLists.txt" "${BUILD_HOST}:${TEMP}/lua/"
...
"${CMAKE}" \\
    ...
    -G "${GENERATOR}" \\
    -DCMAKE_BUILD_TYPE=Release \\
    -DCLINGODL_BUILD_SHARED=OFF \\
    -DCLINGO_BUILD_WITH_PYTHON=OFF \\
    -DCLINGO_BUILD_APPS=OFF \
    -DCLINGO_REQUIRE_LUA=ON \\
    -DCLINGODL_BUILD_STATIC=ON \\
...
The \`clingo-dl.exe\` binary is compiled with Lua 5.3 but
without Python support.

- \`clingo-dl.exe\`: solver for non-ground programs
EOF
...
if [[ ! -e ${SRC} ]]; then
    git clone --branch "$branch" --single-branch --depth=1 git@github.com:potassco/clingoDL ${SRC}
